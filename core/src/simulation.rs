...
enum CacheBackend {
    Moka(Cache<String, SimulationResult>),
...
impl SimulationCache {
    pub async fn new(redis_url: &str) -> Result<Arc<Self>, SimulationError> {
        let client = redis::Client::open(redis_url)?;
        let conn = ConnectionManager::new(client).await?;
        Ok(Arc::new(Self {
            backend: CacheBackend::Redis(Arc::new(tokio::sync::Mutex::new(conn))),
    ...
    pub fn new_in_memory() -> Arc<Self> {
        let inner = Cache::builder()
            ...
            .build();
        Arc::new(Self {
            backend: CacheBackend::Moka(inner),
            hits: AtomicU64::new(0),
            misses: AtomicU64::new(0),
        })
    }
            ...
            CacheBackend::Redis(conn) => {
                ...
                if let Some(json) = json {
                    match serde_json::from_str(&json) {
                        ...
                        Err(e) => {
                            tracing::warn!(cache.key = %key, error = %e, "Failed to deserialize cached value");
                            self.misses.fetch_add(1, Ordering::Relaxed);
                            None
                        }
                    }
        ...
        match &self.backend {
            ...
            CacheBackend::Redis(conn) => {
                if let Ok(json) = serde_json::to_string(&value) {
                    let mut conn = conn.lock().await;
                    let _: Result<(), _> = conn.set_ex(&key, json, CACHE_TTL_SECS).await;
                } else {
                    tracing::warn!("Failed to serialize SimulationResult for caching");
                }
            }
        }
    }
...
mod tests {
    ...
    mod cache_tests {
        ...
        async fn test_cache_miss_on_empty() {
            let cache = SimulationCache::new_in_memory();
